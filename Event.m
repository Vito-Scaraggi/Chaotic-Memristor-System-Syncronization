...
% Class that implements static methods for controlling app events
...
classdef Event 
    methods(Access = public, Static)
        ...
        % Button pushed function: CLEARANIMATIONButton
        ...
        function STARTANIMATIONButtonPushed(app, event)
            ...
            if(strcmp(app.GraphicsDropDown_2.Value,'3D plots') && flag)
                ...
                j1 = index1;
                j2 = index2;
                s = Util.step(step_number);
                t = floor(1:s:step_number);
                
                if t(end) ~= step_number
                    t(end+1) = step_number;
                end
                ...
                if flag
                    l = uilabel(q,'FontColor','#ff3333','Text',' >>> APP ERROR : Unavailable animation. Select 3D plots','VerticalAlignment','center');
                else
                    l = uilabel(q,'FontColor','#ff3333','Text',' >>> APP ERROR : State of the program has been changed. Press SUBMIT button first','VerticalAlignment','center');
                end
        ...
        function SUBMITButtonPushed(app, event)
            ...
            if abs(time_diff) > step_number
                ...
                l = uilabel(g,'FontColor','#ff3333','Text',' >>> PARAM ERROR : time delta cannot be greater than step number','VerticalAlignment','top');
                new = app.UIFigure.Position;
                old = [0.25 0.204 0.5 0.593];
                oldFontSize = l.FontSize;
                coeff = (new(3)/old(3)+new(4)/old(4))/2;
                ...
                if state.wException
                    ...
                    g.RowSpacing = 1;
                    g.BackgroundColor = [0.149 0.149 0.149];
                    l = uilabel(g,'FontColor','#ff3333','Text',' >>> MATH ERROR : function W is undefined for w = ±1','VerticalAlignment','top');
                    new = app.UIFigure.Position;
                    old = [0.25 0.204 0.5 0.593];
                    oldFontSize = l.FontSize;
                    coeff = (new(3)/old(3)+new(4)/old(4))/2;
